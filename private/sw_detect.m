function [SW] = sw_detect(cfg, data)
%SW_DETECT detect slow waves following Massimini, 2004
% based on 3 criteria:
% 1. less than negthr
...
%
%  .chan.negpeak_itrl = first negative peak in samples, from beginning of the
%                   trial (need not be the largest)
%  .chan.negpeak_iabs = first negative peak in samples, based on sampleinfo
%  .chan.negpeak_time = first negative peak in seconds, based on data.time
%  .chan.negpeak_val = min values for all the channels
%  .chan.channel = order of channels, based on their negative peaks
%  .chan.label = same as channel, with labels
%  .chan.delay = delay from first negetive peak
%  .chan.maxdelay = maxdelay
%
...
for i = 1:numel(SW)
  ...
  SW(i).chan.negpeak_itrl = SW(i).begsw_itrl + min(sdlay);
  SW(i).chan.negpeak_iabs = SW(i).begsw_iabs + min(sdlay);
  SW(i).chan.negpeak_time = data.time{SW(i).trl}(SW(i).chan.negpeak_itrl);
    
  SW(i).chan.channel = channel(schan);
  SW(i).chan.label = label(schan);
  SW(i).chan.delay = (sdlay - min(sdlay)) / data.fsample;
  SW(i).chan.negpeak_val = min_v(SW(i).chan.channel);
  SW(i).chan.maxdelay = max(SW(i).chan.delay);
  %-------%
  %-----------------%
  ...
  %-------%
  %-get the data
  datsel = SW(i).negpeak_itrl:SW(i).pospeak_itrl; % from negative peak to positive peak
  x = data.trial{SW(i).trl}(SW(i).chan.channel, datsel); % only for channels that went below cfg.trvlnegthr. Order is important
  %-------%
  
  for j = 1:size(x,1)
    j_zcr = find(x(j,:) >= 0, 1); % reference to negpeak_itrl
    
    %- if it doesn't cross zero, use the last value
    if isempty(j_zcr)
      j_zcr = size(x,2);
    end
    
    j_zcr = j_zcr + (SW(i).negpeak_itrl - SW(i).begsw_itrl); % add offset between negpeak and begsw
    
    chan_time = j_zcr / data.fsample - SW(i).chan.delay(j); % remove the channel-specific delay 
    SW(i).chan.slope(j,1) = SW(i).chan.negpeak_val(j) / chan_time;
  end
  %-----------------%
